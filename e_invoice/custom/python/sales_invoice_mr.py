...
from e_invoice.e_invoice.doctype import e_invoice_item_wise_tax_details
import frappe
import json
...
def login(new_auth_key=False):
    token = frappe.db.get_value(
        "Custom E Invoice SettingsMr", "Custom E Invoice SettingsMr", "auth_token")
    enable = frappe.db.get_single_value('Custom E Invoice SettingsMr', 'enable')
    if not enable:
        frappe.throw(_("Enable {0}.").format(get_link_to_form(
            "Custom E Invoice SettingsMr", "Custom E Invoice SettingsMr")))

    if not new_auth_key:
        exp_time = frappe.db.get_single_value(
            'Custom E Invoice SettingsMr', 'token_expiry')
        if exp_time:
    ...
    client_id = frappe.db.get_single_value(
        'Custom E Invoice SettingsMr', 'client_id')
    client_secret = get_decrypted_password('Custom E Invoice SettingsMr', 'Custom E Invoice SettingsMr',
                                           fieldname='client_secret', raise_exception=False)

    if not client_id and client_secret:
        frappe.throw(_("Kindly provide client credentials in {} .").
                     format(get_link_to_form("Custom E Invoice SettingsMr", "Custom E Invoice SettingsMr")))
    # Get auth token
    # prepod
    ...
    try:
        ...
        if response.status_code in [200, 202]:
            ...
            frappe.db.set_value("Custom E Invoice SettingsMr",
                                None, "auth_token", res['access_token'])
            frappe.db.set_value("Custom E Invoice SettingsMr", None, "token_expiry", add_to_date(
...
def post_invoice(docnames, e_invoice_log=""):
    ...
    try:
        ...
        for doc in doc_list:
            ...
            e_invoice_item = sales_invoice.e_invoice_item
            ...
            # Company Data
            id = frappe.db.get_single_value('Custom E Invoice SettingsMr', 'id')
            name = frappe.db.get_single_value(
                'Custom E Invoice SettingsMr', 'name1')
            actcode = frappe.db.get_single_value(
                'Custom E Invoice SettingsMr', 'activity_code')
            ci = frappe.db.get_single_value(
                'Custom E Invoice SettingsMr', 'region_city')
            st = frappe.db.get_single_value(
                'Custom E Invoice SettingsMr', 'street')
            bn = frappe.db.get_single_value(
                'Custom E Invoice SettingsMr', 'building_number')
            ...
            invoiceLines = []
            if sales_invoice.table == "Items":
             for row in items:
                soup = BeautifulSoup(row.description)
                invoiceLines.append(dict(description=soup.get_text(), itemType=row.item_type, itemCode=row.code, unitType=row.tax_uom if row.tax_uom else row.uom, quantity=row.qty, internalCode='Quest Fees For Training', salesTotal=row.base_amount, total=row.total_amount, valueDifference=0,
                                         totalTaxableFees=0, netTotal=row.base_net_amount, itemsDiscount=row.discount_after));
            else :
                for row in e_invoice_item :
                 soup = BeautifulSoup(row.description)
                invoiceLines.append(dict(description=soup.get_text(), itemType=row.item_type, itemCode=row.code, unitType=row.tax_uom if row.tax_uom else row.uom, quantity=row.qty, internalCode='Quest Fees For Training', salesTotal=row.base_amount, total=row.total_amount, valueDifference=0,
                                         totalTaxableFees=0, netTotal=row.base_net_amount, itemsDiscount=row.discount_after));
    ...
    # Make request
            ...
            payload = json.dumps({
                ...
                "receiver": {
                    ...
                    "id": sales_invoice.tn_id if sales_invoice.customer_types in ['B', 'F'] else "",
                    "name": sales_invoice.invoiced_name if sales_invoice.invoiced_name else sales_invoice.customer_name},
                "documentType": "I",
                "documentTypeVersion": "1.0",
                "dateTimeIssued": f'{sales_invoice.posting_date.strftime("%Y-%m-%d")}T{time}'+'Z',
                # "dateTimeIssued": "2022-11-12T23:59:59Z",
            ...
            for row in items:
                ...
                invoiceLines.append(
                    dict(
                        ...
                        quantity=-(row.qty),
                        internalCode='Quest Fees For Training',
                        salesTotal=-(row.base_amount),
                        total=-(row.total_amount),
                        valueDifference=0,
                        totalTaxableFees=0,
                        netTotal=-(row.base_net_amount),
...
def get_signatured_data(payload, doc, headers):
    try:
        url = frappe.db.get_value(
            "Custom E Invoice SettingsMr", "Custom E Invoice SettingsMr", "electronic_signature_device_endpoint")
